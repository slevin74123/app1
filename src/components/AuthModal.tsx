...
export const AuthModal: React.FC<AuthModalProps> = ({ 
  ...
  const handleSocialAuth = async (method: string, authFunction: () => Promise<{ error: any }>) => {
    ...
    try {
      ...
      if (error) {
        toast.error(`Eroare la autentificarea cu ${method}: ${error.message}`);
      } else {
        toast.success(`Autentificare cu ${method} reușită!`);
        onClose();
      }
    } catch (error) {
      toast.error(`Eroare neașteptată: ${error}`);
    } finally {
      setIsLoading(false);
      setAuthMethod('');
    }
  ...
  const handleEmailAuth = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        toast.success(activeTab === 'signup' 
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-center text-2xl font-bold">
            {activeTab === 'signin' ? 'Autentificare' : 'Înregistrare'}
          </DialogTitle>
        </DialogHeader>
          ...
          <TabsContent value="signin" className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <Button
                variant="outline"
                className="w-full"
                onClick={() => handleSocialAuth('Instagram', signInWithInstagram)}
                disabled={isLoading}
              >
